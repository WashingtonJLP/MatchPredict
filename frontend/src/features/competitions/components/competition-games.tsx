...
export function CompetitionGames({ competitionId }: { competitionId: string }) {
  ...
  const gamesQuery = useDailyGames(selectedDate, { competition: competitionId });
    ...
    <div className="space-y-4">
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,28rem)] lg:items-center">
        <div className="min-w-0">
          <h3 className="text-lg font-extrabold text-card-foreground">Agenda da competição</h3>
          <p className="mt-1 text-sm font-semibold text-muted-foreground">
            {formatLongDate(selectedDate)} · horário de Brasília
          </p>
        </div>
        <div className="min-w-0 [&_input]:min-w-0 [&_label]:min-w-0">
          <DailyGamesDateControl selectedDate={selectedDate} onDateChange={updateDate} />
        </div>
      </div>
      ...
      {gamesQuery.isLoading ? (
        ...
        <div className="space-y-4">
          <ErrorState
            ...
            description={getApiErrorMessage(gamesQuery.error, "Tente novamente em instantes.")}
          />
          <div className="flex justify-center">
            <Button type="button" className="h-11 rounded-xl" onClick={() => void gamesQuery.refetch()}>
