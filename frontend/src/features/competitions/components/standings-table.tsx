...
import { formatStandingValue } from "@/features/competitions/competition-view";
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm shadow-primary/5">
      {showTitle ? (
        <header className="border-b border-border bg-secondary/50 px-4 py-3 sm:px-5">
          <h3 className="text-lg font-extrabold text-card-foreground">{section.name}</h3>
        </header>
      ) : null}
      <div className="w-full">
        <table className="w-full table-fixed border-collapse text-sm sm:table-auto">
          ...
          <thead className="bg-muted/60 text-xs font-extrabold uppercase tracking-wide text-muted-foreground">
            <tr>
              <th scope="col" className="w-8 px-1 py-3 text-center sm:w-14 sm:px-3">POS</th>
              <th scope="col" className="overflow-hidden px-1 py-3 text-left sm:px-3">Time</th>
              <StatHeading label="J" />
              ...
              <th scope="col" className="w-9 px-0.5 py-3 text-center text-foreground sm:w-16 sm:px-3">PTS</th>
            </tr>
          </thead>
            ...
            {section.entries.map((entry) => (
              <tr key={entry.team.id} className="relative hover:bg-muted/35">
                <td className="relative w-8 px-1 py-3 text-center font-extrabold tabular-nums text-muted-foreground sm:w-14 sm:px-3">
                  {entry.zone ? (
                    <span
                      ...
                      title={entry.zone.description}
                      aria-label={entry.zone.description}
                    />
                  ) : null}
                  {entry.position}
                </td>
                <th scope="row" className="min-w-0 overflow-hidden px-1 py-2.5 text-left sm:px-3">
                ...
                <td className="w-9 px-0.5 py-3 text-center text-base font-extrabold tabular-nums text-primary sm:w-16 sm:px-3">
                  ...
                  {entry.deductions > 0 ? (
                    <span className="ml-0.5 text-xs text-destructive" title={`${entry.deductions} ponto(s) deduzido(s)`}>
                      *
                    </span>
                  ) : null}
                </td>
      ...
      {zones.length > 0 ? (
        <footer className="flex flex-wrap gap-x-4 gap-y-2 border-t border-border bg-muted/30 px-4 py-3 text-xs font-semibold text-muted-foreground">
          <span className="inline-flex items-center gap-1.5">
            <Info className="size-3.5" aria-hidden /> Zonas informadas pela ESPN:
          </span>
          {zones.map((zone) => (
            <span key={zone} className="inline-flex items-center gap-1.5">
              <span className="size-2 rounded-full bg-accent" aria-hidden /> {zone}
            </span>
          ))}
        </footer>
      ) : null}
...
function StatHeading({ label, optional = false }: { label: string; optional?: boolean }) {
  return (
    <th
      scope="col"
...
function StatCell({ value, optional = false }: { value: number | null; optional?: boolean }) {
  return (
    <td
...
function uniqueZones(section: StandingSection) {
  return [...new Set(section.entries.flatMap((entry) => (entry.zone ? [entry.zone.description] : [])))];
}
